...
const std::vector<float>& seeded_cached(size_t n, unsigned seed) {
    ...
    if (it == cache.end()) {
        if (cache.size() > 256) {
            cache.clear(); /* bound growth across queries */
        }
...
void gpu_nmf_batched(const float* dV, int M, int N, float* dW, float* dH, int R, int P, int iters,
                     int n_fixed, ScoreContext& ctx) {
...
std::vector<LengthData> band_stats(const float* dH, const CandidateTemplates& ct, int Ns,
                                   ScoreContext& ctx) {
    ...
    checkCuda(
        cudaMemcpy(Hs.ptr(), dH, (size_t)P * R * Ns * sizeof(float), cudaMemcpyDeviceToDevice));
    ...
    for (int p0 = 0; p0 < P; p0 += pchunk) {
        ...
        launch_distance_batched(ct.Zo.ptr(), No, Zs.ptr() + (size_t)p0 * K * Ns, Ns, K, pc,
                                D.ptr());
        for (auto& ld : lens) {
...
MatchInfo score_candidate_from_H(const float* dH, const CandidateTemplates& ct, int Ns, bool clip,
                                 ScoreContext& ctx, std::vector<float>* sel_out = nullptr,
    ...
    for (size_t li = 0; li < lens.size(); li++) {
            ...
            for (int p = 0; p < P; p++) {
                ...
                if (getenv("SD_DEBUG") && sel[it] < lbest.score * 1.05f) {
                    fprintf(stderr, "    T=%ds p=%+.2f w@%.0fs raw=%.4f sel=%.4f\n", window_seconds,
                            pitch_shift(p), w * hop / fps, s[it], sel[it]);
                }
        ...
        if (li == 0 && sel_out) {
            *sel_out = sel; /* pre-median-normalized copy */
        }
        if (li == 0 && raw_out) {
            *raw_out = s;
        }
        ...
        if (window_seconds == WINDOW_SECONDS.front()) {
            best = lbest;
        }
...
std::vector<MatchInfo> gpu_score_candidates(const GpuMat& Vq,
    ...
    for (int c = 0; c < C; c++) {
        out[c] = score_candidate_from_H(ctx.H_all.ptr() + (size_t)c * P * R * Ns, *cts[c], Ns, clip,
                                        ctx);
    }
    return out;
}

MatchInfo gpu_score_candidate(const GpuMat& Vq, const CandidateTemplates& ct, int iters,
                              bool clip) {
    ScoreContext ctx;
...
BtPath backtrack_path(const unsigned char* pred, int T, int Ns, int je, float cost, float len) {
    ...
    const float dy = (float)(je - out.js);
    const float dx = (float)(T - 1);
...
std::vector<PathFeatures> gpu_extract_features(const GpuMat& Vq, const CandidateTemplates& ct,
                                               int iters, bool clip, int top_f, ScoreContext& ctx) {
    const int M = ANALYSIS_BINS, Ns = Vq.cols;
    const int K = RANK_K, R = RANK_K + RANK_L, P = N_SHIFTS;
    const int No = ct.No;
    const float fps = (float)SAMPLE_RATE / HOP;
    ...
    for (auto& [p, ws] : by_shift) {
        launch_distance_batched(ct.Zo.ptr(), No, ctx.Zs.ptr() + (size_t)p * K * Ns, Ns, K, 1,
                                ctx.D.ptr());
        ...
        launch_dtw_band_preds(No, T, Ns, F, dws.ptr(), hop, ctx.D.ptr(), dpred.ptr(), dlast.ptr());
